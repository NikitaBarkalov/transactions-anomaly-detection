...
class UserAggregator:
    ...
    def fit(self, df: pd.DataFrame) -> "UserAggregator":
        ...
        df["_is_mismatch"] = (df["ip_country"].astype(str).str.upper() != df["bin_country"].astype(str).str.upper()).astype(float)
        ...
        aggs["user_n_currencies"] = df.drop_duplicates(subset=["user_id", "currency"]).groupby("user_id").size()
        aggs["user_n_pay_methods"] = df.drop_duplicates(subset=["user_id", "payment_method"]).groupby("user_id").size()
        aggs["user_n_banks"] = df.drop_duplicates(subset=["user_id", "bank_id"]).groupby("user_id").size()
        aggs["user_n_psps"] = df.drop_duplicates(subset=["user_id", "psp_id"]).groupby("user_id").size()

        self.user_stats_ = aggs.reset_index()
        self.feature_cols = [c for c in self.user_stats_.columns if c != "user_id"]
        self.fallback_values_ = {col: float(self.user_stats_[col].median()) for col in self.feature_cols}
        return self
...
def _fast_grouped_rolling_mean(user_ids: np.ndarray, values: np.ndarray, window: int = 10, default_fill: float = 0.0) -> np.ndarray:
    n = len(values)
...
def compute_sequential_user_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["user_rolling_fail_rate_10"] = _fast_grouped_rolling_mean(user_ids, is_fail, window=10, default_fill=0.0)

    amount_usd = df["amount_usd"].values if "amount_usd" in df.columns else df["amount"].values.astype(np.float64)
    global_mean_amt = float(np.mean(amount_usd))
    user_hist_mean = _fast_grouped_rolling_mean(user_ids, amount_usd, window=10, default_fill=global_mean_amt)
